...
from app.schemas.household import (
    HouseholdCreate, HouseholdResponse, HouseholdWithMembers,
    InviteCreate, InviteResponse, JoinHouseholdRequest,
    MemberRoleUpdate, MemberWithUser
)
...
def list_my_households(
    ...
    for membership in memberships:
        ...
        households.append(HouseholdResponse(
            id=household.id,
            name=household.name,
            created_by=household.created_by,
            created_at=household.created_at,
...
def join_household(
    ...
    invite = db.query(HouseholdInvite).filter(
        HouseholdInvite.token == join_data.token
    ).first()
...
def update_member_role(
    ...
    if member.role == MemberRole.OWNER and role_data.role == MemberRole.MEMBER:
        owner_count = db.query(HouseholdMember).filter(
            HouseholdMember.household_id == household_id,
